                ...
                if feature.type == "gene":
                    ...
                    if isinstance(feature.location, CompoundLocation) and any(
                        ...
                        if feature.location.strand == 1:
                            upstream_start = int(adj_start - 205)
                            upstream_end = int(adj_start - 95)
                        elif feature.location.strand == -1:
                            upstream_start = int(adj_end + 95)
                            upstream_end = int(adj_end + 205)

                        # Iterate over the adjusted range
                        for position in range(upstream_start, upstream_end):
                        ...
                        for part_location in locations:
                            ...
                            if feature.location.strand == 1:
                                upstream_start = int(part_location.start - 205)
                                upstream_end = int(part_location.start + 95)
                            elif feature.location.strand == -1:
                                upstream_start = int(part_location.end + 95)
                                upstream_end = int(part_location.end + 205)

                            for position in range(
                                int(upstream_start), int(upstream_end)
                            ):
                                key = (record.id, position)
                                locus_map.setdefault(key, []).append(
                                    (
            ...
            if (
                ...
                if not aligned_genes:
                    ...
                    for (
                        ...
                        rows_copy.update(
                            {
                                "downstream_locus": locus_tag,
                                "downstream_gene": (
                                    gene_name if gene_name else locus_tag
                                ),
                                "offset": offset,
                                "overlap": overlap,
...
def create_downstream_note(row):
    ...
    if row["sites"] > 0:
        ...
        if row["genes"] > 0:
            parts.append(
                f"{row['genes']} {'gene downstream' if row['genes'] == 1 else 'genes downstream'}"
            )
        if row["intergenic"] > 0:
...
def main(args):
    ...
    try:
        ...
        gene_counts = results.loc[
            results["downstream_locus"].notnull(), "spacer"
        ].value_counts()
        intergenic_counts = results.loc[
            results["downstream_locus"].isnull() & results["target"].notnull(), "spacer"
        ].value_counts()
        ...
        column_order = ["spacer", "downstream_locus", "downstream_gene", "chr"]
    ...
    unique_tags = results["downstream_locus"].nunique()
    ...
    overlapping_genes_targeted = results.loc[
        results["genes"] > 1, "downstream_locus"
    ].nunique()
    combined_table.add_row(
    ...
    intergenic_spacers = results[
        (results["downstream_locus"].isnull()) & (results["chr"].notnull())
    ]["spacer"].nunique()
    combined_table.add_row(
