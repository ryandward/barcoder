...
def read_in_chunks(file1, file2=None, chunk_size=2**16):

    reads_forward, reads_revcomp = [], []
        ...
        while True:
            try:
                if file_type == 'fastq':
                    ...
                    reads_forward.append(next(iters[0]).strip())  # Capture SEQUENCE
                    next(iters[0])  # Skip +
                    next(iters[0])  # Skip QUALITY

                    if file2:
                        next(iters[1])  # Skip @SEQUENCE_ID
                        reads_revcomp.append(next(iters[1]).strip())  # Capture SEQUENCE
                        next(iters[1])  # Skip +
                ...
                if len(reads_forward) >= chunk_size:
                    yield (reads_forward[:chunk_size], reads_revcomp[:chunk_size] if reads_revcomp else None)
                    reads_forward, reads_revcomp = reads_forward[chunk_size:], reads_revcomp[chunk_size:] if reads_revcomp else []
                    
            except StopIteration:
                break
                
        if reads_forward:
            yield (reads_forward, reads_revcomp if reads_revcomp else None)
...
def find_ends(reads: List[str], start: int, length: int, revcomp: bool = False) -> Tuple[str, str]:

    # Initialize max values; reverse if needed
    max_left, max_right = (50, 50) if not revcomp else (50, 50)[::-1]
...
def process_chunk(chunk, barcodes_with_junctions_forward, barcodes_with_junctions_revcomp, junction_start1, junction_start2, barcode_length, left1, right1, left2, right2, need_swap):
    
    if need_swap:
        reads_revcomp, reads_forward = chunk
    else:
        reads_forward, reads_revcomp = chunk


    counts = defaultdict(int)

    def validate_candidate_with_junction(candidate_with_junction, left, right, revcomp=False, check_set=True):
        if not check_set:
           in_barcodes_with_junctions = False
        else:
           in_barcodes_with_junctions = candidate_with_junction in barcodes_with_junctions_forward
        
        if revcomp:
            candidate = candidate_with_junction[safe_len(right):safe_len(candidate_with_junction) - safe_len(left)][::-1].translate(str.maketrans("ATCGN", "TAGCN"))
        else:
            candidate = candidate_with_junction[safe_len(left):safe_len(candidate_with_junction) - safe_len(right)]
        
        has_junction = candidate_with_junction.startswith(left or '') and candidate_with_junction.endswith(right or '')
        
        return in_barcodes_with_junctions, has_junction, candidate
    ...
    if reads_forward and reads_revcomp:  # Paired-end processing
        for record_forward, record_revcomp in zip(reads_forward, reads_revcomp):
            if 'N' in record_forward:
                continue
            candidate_with_junction1 = record_forward[junction_start1:junction_start1 + left1_len + barcode_length + right1_len]
            in_barcodes_with_junctions1, has_junction1, candidate1 = validate_candidate_with_junction(candidate_with_junction1, left1, right1)

            candidate_with_junction2 = record_revcomp[junction_start2:junction_start2 + left2_len + barcode_length + right2_len]

            # instead of checking if candidate2 is a barcode (it isn't, because its reverse-complemented), compare to the forward read
            in_barcodes_with_junctions2, has_junction2, candidate2 = validate_candidate_with_junction(candidate_with_junction2, left2, right2, revcomp=True, check_set=False)

            if candidate1 != candidate2: 
                continue
            if in_barcodes_with_junctions1 and has_junction1 and has_junction2:
                counts[candidate1] += 1
            elif has_junction1 and has_junction2:
                counts[candidate1 + "*"] += 1


    elif reads_forward:  # Single-end processing, forward orientation
        for record_forward in reads_forward:
            if 'N' in record_forward:
                continue
            candidate_with_junction1 = record_forward[junction_start1:junction_start1 + left1_len + barcode_length + right1_len]
            in_barcodes_with_junctions1, has_junction1, candidate1 = validate_candidate_with_junction(candidate_with_junction1, left1, right1, revcomp=False, check_set=True)
            
            # easiest mode: barcodes_with_junctions are in the forward orientation and have no paired-end reads
            if in_barcodes_with_junctions1 and has_junction1:
                counts[candidate1] += 1
            elif has_junction1:
                counts[candidate1 + "*"] += 1
            

    elif reads_revcomp:  # Single-end processing, reverse complement
        for record_revcomp in reads_revcomp:
            if 'N' in record_revcomp:
                continue
            candidate_with_junction2 = record_revcomp[junction_start2:junction_start2 + left2_len + barcode_length + right2_len]
            in_barcodes_with_junctions2, has_junction2, candidate2 = validate_candidate_with_junction(candidate_with_junction2, left2, right2, revcomp=True, check_set=True)
            
            # reads will be reverse complemented and checked with 
            if in_barcodes_with_junctions2 and has_junction2:
                counts[candidate2] += 1
            elif has_junction2:
                counts[candidate2 + "*"] += 1
    
    if reads_forward and reads_revcomp and len(reads_forward) != len(reads_revcomp):
        raise ValueError("Length of reads_forward and reads_revcomp should be the same for paired-end data.")

    return (counts, len(reads_forward) if reads_forward else len(reads_revcomp))


def find_start_positions(reads, barcodes, barcode_length, revcomp=False):
...
def main(args):
    ...
    reads_forward = args.file1
    reads_revcomp = args.file2
    ...
    new_reads_sampled, need_swap, sample1, sample2 = sample_data(reads_forward, reads_revcomp, barcodes, is_paired_end)
    ...
    if need_swap:
        ...
        reads_forward, reads_revcomp = reads_revcomp, reads_forward
    ...
    if sample2:
        ...
        barcode_start2 = find_start_positions(sample2, barcodes, barcode_length, revcomp=True)
    ...
    if sample2 is not None:
        ...
        left2, right2 = find_ends(sample2, barcode_start2, barcode_length, revcomp=True)
   ...
   # Calculate reverse complements
    ...
    def add_junction(barcodes, left=None, right=None, revcomp=False):
        if revcomp:
            barcodes = {barcode[::-1].translate(str.maketrans("ATCGN", "TAGCN")) for barcode in barcodes}
        left, right = (left or ""), (right or "")
        return {left + barcode + right for barcode in barcodes}
    
    # Add junctions to the ends of the barcodes, if present

    barcodes_with_junctions_forward = add_junction(barcodes, left1, right1)
    barcodes_with_junctions_revcomp = add_junction(barcodes, left2, right2, revcomp=True)

    console.log("Executing high-throughput read analysis...")
    chunk_generator = read_in_chunks(args.file1, args.file2 if is_paired_end else None)

    # Create argument generator
    args_generator = ((chunk, barcodes_with_junctions_forward, barcodes_with_junctions_revcomp, junction_start1, junction_start2, barcode_length, left1, right1, left2, right2, need_swap) for chunk in chunk_generator)
