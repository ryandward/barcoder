...
def sorter(input_queues, sorted_chunks_queue, read_complete_events, sort_complete_event):
...
def merger(sorted_chunks_queue, output_queues, sort_complete_events):
...
if __name__ == "__main__":
    num_sorters = 8  # Or any other number based on available resources
    ...
    merger_process = multiprocessing.Process(target=merger, args=(sorted_chunks_queue, output_queues, sort_complete_events))
    ...
    sorters = [multiprocessing.Process(target=sorter, args=(queues, sorted_chunks_queue, read_complete_events, sort_complete_events[i])) for i in range(num_sorters)]
